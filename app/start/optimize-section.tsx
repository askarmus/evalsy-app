  ...
  const data = [
    {
      ...
      steps: [
        { icon: <BarChart3 className="h-6 w-6 text-[#3534ff]" />, title: 'Analyze Costs', desc: 'Track traditional hiring expenses' },
        { icon: <Zap className="h-6 w-6 text-[#3534ff]" />, title: 'AI Automation', desc: 'Deploy smart screening tools' },
        { icon: <TrendingUp className="h-6 w-6 text-[#3534ff]" />, title: 'Save Money', desc: 'Reduce cost per hire by 80%' },
      ],
      comparison: {
    ...
    },
    {
      ...
      steps: [
        { icon: <Users className="h-6 w-6 text-[#3534ff]" />, title: 'Instant Screening', desc: 'AI reviews resumes in seconds' },
        { icon: <CheckCircle className="h-6 w-6 text-[#3534ff]" />, title: 'Quick Matching', desc: 'Find perfect candidates fast' },
        { icon: <Zap className="h-6 w-6 text-[#3534ff]" />, title: 'Rapid Hiring', desc: 'Complete process in 10 days' },
      ],
      comparison: {
    ...
    },
    {
      ...
      steps: [
        {
          icon: <BarChart3 className="h-6 w-6 text-[#3534ff]" />,
          title: 'Skill Analysis',
          desc: 'Deep dive into candidate abilities',
        },
        { icon: <Target className="h-6 w-6 text-[#3534ff]" />, title: 'Perfect Match', desc: 'AI finds ideal culture fit' },
        { icon: <CheckCircle className="h-6 w-6 text-[#3534ff]" />, title: 'Quality Hire', desc: '99% accuracy in matching' },
      ],
      comparison: {
    ...
    },
    {
      ...
      steps: [
        { icon: <Users className="h-6 w-6 ttext-[#3534ff]" />, title: 'Bulk Processing', desc: 'Handle hundreds of resumes' },
        { icon: <Zap className="h-6 w-6 text-[#3534ff]" />, title: 'Smart Sorting', desc: 'AI ranks candidates instantly' },
        { icon: <TrendingUp className="h-6 w-6 text-[#3534ff]" />, title: 'Scale Up', desc: 'Process 100+ candidates daily' },
      ],
      comparison: {
    ...
    <section id="cost-analysis" className="w-full py-10 md:py-16 lg:py-20   overflow-hidden">
      <div className="container px-4 md:px-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {data.map((item) => (
            <Button key={item.id} variant={activeTab === item.id ? 'solid' : 'bordered'} onPress={() => setActiveTab(item.id)} className={`px-6 py-2 text-sm font-medium border-2 rounded-full transition-all flex items-center gap-2 ${activeTab === item.id ? 'bg-[#3534ff] text-white border-[#3534ff] hover:bg-[#3534ff]' : 'bg-transparent text-black border-black hover:border-[#98FB98] hover:bg-[#98FB98] hover:text-black'}`}>
              {item.icon}
              {item.label}
            </Button>
          ))}
          ...
          <Card className="bg-white border-2 border-black rounded-3xl mb-6">
            <CardBody className="p-6 md:p-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 relative">
                {currentData.steps.map((step, index) => (
                  <div key={index} className="flex flex-col items-center text-center">
                    <div className="rounded-full   p-4 mb-4 border-2 border-[#3534ff]">{step.icon}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Traditional */}
                <div className="text-center p-6 bg-gradient-6 rounded-xl border-2 border-gray-200">
                ...
                {/* AI */}
                <div className="text-center p-6 bg-[#3534ff] rounded-xl  ">
