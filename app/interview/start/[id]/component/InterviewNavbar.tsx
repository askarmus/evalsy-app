...
const InterviewNavbar: React.FC<InterviewNavbarProps> = ({ company, totalMinutes, startTime, onInterviewComplete }) => {
  return (
    <nav className='flex items-center justify-between w-full bg-white'>
      ...
      <div className='flex items-center'>{company.logo ? <Image src={company.logo} alt={`${company.name} Logo`} width={100} height={40} className='w-auto min-h-[30px] max-h-[40px] object-contain' /> : <p className='font-bold text-lg text-gray-800'>{company.name}</p>}</div>
