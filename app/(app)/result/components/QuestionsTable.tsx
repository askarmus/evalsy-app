          ...
          <li key={answer.id || `question-${index}`} className='rounded-lg group cursor-pointer' onClick={() => handleClick(answer)}>
            <div className='flex flex-row'>
              <div className='items-center flex flex-col justify-around'>
                ...
                <div className='bg-green-400 border-2 border-gray-400 rounded-full h-4 w-4 flex flex-grow justify-around'>
                  <svg className='flex-none m-2 w-4 h-4 opacity-0 group-hover:opacity-100' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
              ...
              <div className='flex flex-col group-hover:bg-white ml-1 p-2 pr-6 rounded-xl'>
                <div className={`ml-4 text-sm ${isSelected ? "font-bold" : ""}`}>{answer.text}</div>
              </div>
            </div>
          </li>
        );
